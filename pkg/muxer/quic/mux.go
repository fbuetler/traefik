...
var quicFunc = map[string]func(*matchersTree, ...string) error{
	"HostSNI":       hostSNI,
...
// This is a first naive implementation used in QUIC routing.
func ParseHostSNI(rule string) ([]string, error) {
	var matchers []string
	for matcher := range quicFunc {
...
// ConnData contains QUIC connection metadata.
...
func NewConnData(serverName string, conn net.Conn, alpnProtos []string) (ConnData, error) {
	remoteIP, _, err := pan.SplitHostPort(conn.RemoteAddr().String())
	if err != nil {
...
// Muxer defines a muxer that handles QUIC routing with rules.
type Muxer struct {
	routes []*route
	parser predicate.Parser
}

// NewMuxer returns a QUIC muxer.
func NewMuxer() (*Muxer, error) {
	var matcherNames []string
	for matcherName := range quicFunc {
...
func (m Muxer) Match(meta ConnData) (quic.Handler, bool) {
...
func (m *Muxer) AddRoute(rule string, priority int, handler quic.Handler) error {
...
func addRule(tree *matchersTree, rule *rules.Tree) error {
	...
	default:
		...
		err = quicFunc[rule.Matcher](tree, rule.Value...)
		if err != nil {
			return err
		}
...
// route holds the matchers to match QUIC route,
...
func hostSNI(tree *matchersTree, hosts ...string) error {
	...
	tree.matcher = func(meta ConnData) bool {
		// Since a HostSNI(`*`) rule has been provided as catchAll for non-TLS QUIC,
